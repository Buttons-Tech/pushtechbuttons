  ...
  return (
    <div className="bg-amber-50/60 rounded-3xl p-5 border border-amber-100/50 space-y-4">
      <div className="flex items-center gap-2">
        <div className={`w-2 h-2 rounded-full ${status === 'loading' ? 'bg-amber-500 animate-spin' : 'bg-lime-500 animate-pulse'}`} />
        <p className="text-xs font-semibold text-amber-900">
          {status === 'loading' ? 'Securing your button...' : 'Get early access updates via WhatsApp'}
        </p>
      </div>
      ...
      {status === 'success' ? (
        <div className="bg-white border border-lime-200 rounded-xl p-4 text-center animate-fade-in">
          ...
          <p className="text-xs text-gray-500 mt-1">Your spot is saved. We will ping you on WhatsApp soon.</p>
          <button 
        ...
        <form onSubmit={handleTapIn} className="space-y-3">
          ...
          {status === 'error' && (
            <p className="text-xs font-semibold text-red-600 px-1">{errorMessage}</p>
          )}
