...
import { 
  ...
  RefreshCw, Image as ImageIcon, DollarSign, Utensils 
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-12">
      ...
      <header className="bg-emerald-600 text-white px-6 py-8 shadow-md">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold mt-1">Live Menu Management</h1>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 mt-8">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm mb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700">
              Live Sync: <strong className="text-emerald-600">{items.length} Dishes Active</strong>
            </span>
          </div>
        ...
        {loading && items.length === 0 ? (
          <div className="text-center py-16 text-slate-400">Loading menu items...</div>
        ) : items.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
            <p className="text-slate-500 font-medium">No food items created yet.</p>
            <button
              onClick={() => handleOpenModal()}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Image URL (Real Food Picture)</label>
                <input
                  type="url"
                  required
              ...
              <div>
                <span className="block text-xs font-bold text-slate-500 mb-2">Or choose a sample photo:</span>
                  ...
                  {DEFAULT_FOOD_IMAGES.map((img, idx) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-lg overflow-hidden border-2 flex-shrink-0 ${
                        formData.imageUrl === img ? 'border-emerald-500 ring-2 ring-emerald-200' : 'border-transparent'
                      }`}
                    >
