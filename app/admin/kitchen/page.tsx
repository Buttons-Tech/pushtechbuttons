...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'https://kitchen-server-d763.onrender.com/food';
    ...
    <div className="min-h-screen bg-slate-100 text-slate-800 p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div>
            <h1 className="text-2xl font-black text-slate-900">Kitchen Staff Dashboard</h1>
            <p className="text-xs text-slate-500">Manage real-time food availability, prices, and photos</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Food Photo (Cloudinary)</label>
                <input
                  type="file"
                  accept="image/*"
                ...
                />
                {isUploading && <p className="text-xs text-amber-600 mt-1">Uploading to Cloudinary...</p>}
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <h2 className="text-lg font-bold mb-4 text-slate-900">Current Food Items</h2>

            {isLoading ? (
              <p className="text-xs text-slate-500">Loading menu from NestJS server...</p>
            ) : foods.length === 0 ? (
              <p className="text-xs text-slate-500">No items found in database.</p>
            ) : (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <h3 className="font-bold text-xs text-slate-900 truncate mt-1">{food.name}</h3>
                          <p className="text-xs font-extrabold text-emerald-700">₦{food.price.toLocaleString()}</p>
                        </div>
                      </div>
