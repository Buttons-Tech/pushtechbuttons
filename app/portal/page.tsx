...
const POLICY_DATA: Record<string, RoleDetails> = {
  nysc: {
    ...
    perks: ["Company Accommodation", "Performance Bonuses", "Mentorship & Career Path"],
    description: "Designed for active NYSC corps members ready to work on live products in software engineering, UI/UX, and growth marketing.",
  },
  siwes: {
    title: "SIWES / Student Internship Policy",
    category: "Undergraduate Attachment",
    stipend: "₦25,000 - ₦35,000 / month",
    perks: ["Logistics Support", "Hands-on Product Tasks", "Letter of Recommendation"],
    description: "Structured for university/polytechnic students fulfilling industrial training requirements. Emphasis is on practical skills in Next.js, design systems, and operations.",
  },
  investors: {
    title: "Investor & Partner Access",
    category: "Strategic Growth",
    stipend: "Equity & Revenue Share",
    perks: ["Quarterly Pitch Decks", "Financial Metrics Dashboard", "Direct Founders Desk Access"],
    description: "Dedicated portal for prospective and existing partners backing Buttns urban tech footprint.",
  },
};
...
export default function PortalHub() {
  ...
  const quickActions = [
    { id: "siwes", label: "SIWES / Interns", icon: GraduationCap, color: "bg-emerald-100 text-emerald-600" },
    { id: "nysc", label: "NYSC Corps", icon: Building2, color: "bg-blue-100 text-blue-600" },
    { id: "staff", label: "Core Staff", icon: Briefcase, color: "bg-purple-100 text-purple-600" },
    { id: "investors", label: "Investors", icon: TrendingUp, color: "bg-amber-100 text-amber-600" },
    { id: "partners", label: "Partners", icon: Handshake, color: "bg-rose-100 text-rose-600" },
  ];
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen p-4 font-sans text-gray-800">
      ...
      <div className="bg-black rounded-2xl p-5 text-white shadow-lg mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight">Buttns Urban Tech</h1>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {quickActions.map((action) => {
            ...
            return (
              <button
                key={action.id}
                onClick={() => setSelectedRole(POLICY_DATA[action.id] || POLICY_DATA["siwes"])}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-amber-900">Priority Hiring Active</h3>
          <p className="text-xs text-amber-700 mt-0.5">
            We are actively enrolling NYSC members & SIWES interns across UI/UX, Next.js engineering, and creative design.
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center sm:items-center">
          <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl p-6 shadow-xl animate-in slide-in-from-bottom duration-200">
            ...
            <div className="bg-gray-50 p-3 rounded-xl mb-4 border border-gray-100">
              <span className="text-xs text-gray-500 block mb-1">Monthly Compensation / Stipend</span>
              <span className="text-base font-bold text-emerald-600">{selectedRole.stipend}</span>
            </div>
